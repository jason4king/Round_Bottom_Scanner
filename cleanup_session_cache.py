...
def clean_symbol(repository: ParquetBarRepository, symbol: str, timeframe: str, apply: bool) -> tuple[int, int] | None:
...
def main() -> None:
    ...
    parser.add_argument("--apply", action="store_true", help="Actually rewrite affected Parquet files (default: dry run)")
    ...
    if total_affected == 0:
        ...
        print(f"Cleaned {total_affected} symbol/timeframe caches.")
    else:
