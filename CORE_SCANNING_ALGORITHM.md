...
### 4.8 F8 — RSI W-Bottom（观察型买点）

先确认价格双底，再等待 RSI 动能恢复。确认可使用最近 5 根 K 线内的 RSI 上穿，或“RSI 底背离且 RSI 均线转升”。订单块共振只读取第二低点当时已经形成且仍有效的多头订单块，不使用未来区域回填历史信号。

颈线突破只有同时满足以下质量过滤才标为“买点确认”：

- RSI 不高于 `65`，且收盘价至少高出颈线 `0.5%`；
- 突破 K 线涨幅不超过 `4%`；
- 收盘价高出 EMA12 不超过 `5%`；
- 入场价至结构止损的风险不超过 `7%`；
- RSI 底背离或多头订单块共振至少满足一项。

成交量比率仍会输出用于观察，但不再作为硬性门槛。F8 不参与评分。

对于没有形成两个独立摆动低点的快速 V 型反转，使用独立的“V底确认”分支：RSI10 必须降至 `25` 或以下并创近 10 根最低价。普通路径要求低点距离当时已经存在的多头订单块不超过 `1%`，随后 8 根 K 线内 RSI 上穿均线、收盘突破此前 3 根 K 线最高价并重新站上 EMA12，确认时 RSI 不高于 `50`；当 RSI10 低至 `20` 或以下时，允许使用极端超卖路径，不强制订单块共振或立即站上 EMA12，RSI 上限放宽到 `60`，但必须在 RSI 金叉后收盘突破前一根 K 线最高价。最终仍执行单根涨幅不超过 `4%`、偏离 EMA12 不超过 `5%`、结构风险不超过 `7%` 的追高过滤。V 底和 W 底分别标记，不互相替代。

### 4.9 P1～P6 — PatternPy 经典形态观察器
...
F8 RSI W-Bottom（价格双底、RSI 背离与动能确认；不参与评分）
