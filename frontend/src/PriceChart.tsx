  ...
  useEffect(() => {
    ...
    const drawStructure=()=>{
      ...
      if(showElliott&&payload.elliott_wave.active){
        ...
        if(wavePoints.length>1){context.strokeStyle=waveColor;context.lineWidth=2;for(let index=1;index<wavePoints.length;index+=1){const previous=wavePoints[index-1],current=wavePoints[index];context.setLineDash(previous.point.soft||current.point.soft?[6,5]:[]);context.beginPath();context.moveTo(previous.x!,previous.y!);context.lineTo(current.x!,current.y!);context.stroke()}context.setLineDash([])}
        wavePoints.forEach(({point,x,y})=>{context.fillStyle=point.soft?"#f5c84b":waveColor;context.beginPath();context.arc(x!,y!,9,0,Math.PI*2);context.fill();context.fillStyle="#07131e";context.font="bold 10px Inter, sans-serif";context.textAlign="center";context.fillText(`${point.name}${point.soft?"?":""}`,x!,y!+3)});
        context.fillStyle=waveColor;context.font="bold 11px Inter, Microsoft YaHei, sans-serif";context.textAlign="left";context.fillText(`${wave.direction==="bullish"?"看涨":"看跌"} · ${wave.state_label}${wave.points.some(point=>point.soft)?" · 候选":""}`,12,18);
