...
export function ScoringTab({symbol}:{symbol:string}){
  ...
  const map=new Map<string,{base:number;multiplier:number;score:number}>();data.factors.filter(f=>f.triggered).forEach(f=>{if(!map.has(f.factor_id))map.set(f.factor_id,{base:f.base_score,multiplier:f.multiplier,score:f.contribution})});const contributions=[...map.entries()];
