...
def get_symbol_bars(
    ...
    for timestamp, row in chart.iterrows():
        ...
        payload.append({
            ...
            "bullish_order_block_enhanced_distance_pct": float(row["BULLISH_OB_ENHANCED_DISTANCE_PCT"]) if pd.notna(row["BULLISH_OB_ENHANCED_DISTANCE_PCT"]) else None,
            "bullish_order_block_quality": float(row["BULLISH_OB_QUALITY"]) if pd.notna(row["BULLISH_OB_QUALITY"]) else None,
            "bullish_order_block_retest_score": float(row["BULLISH_OB_RETEST_SCORE"]) if pd.notna(row["BULLISH_OB_RETEST_SCORE"]) else None,
            "bullish_order_block_touch_count": int(row["BULLISH_OB_TOUCH_COUNT"]),
            "bullish_order_block_age": int(row["BULLISH_OB_AGE"]),
            "bullish_order_block_retest_confirmed": bool(row["BULLISH_OB_RETEST_CONFIRMED"]),
            "bullish_order_block_pierced": bool(row["BULLISH_OB_PIERCED"]),
            "bullish_order_block_status": str(row["BULLISH_OB_STATUS"]) if pd.notna(row["BULLISH_OB_STATUS"]) else None,
